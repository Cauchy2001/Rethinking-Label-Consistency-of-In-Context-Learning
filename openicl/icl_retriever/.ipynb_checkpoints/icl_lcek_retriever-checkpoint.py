...
class LcekRetriever(TopkRetriever):
    ...
    def topk_search(self):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            for j in range(self.select_time):
                ...
                NA = {"text": "N/A", "label": 0, "label_text": ""}
                #NA = {"text1": "N/A","text2": "N/A", "label": 0, "label_text": ""}
                for i in range(0, len(labels), batch_size):
                    batch_labels = labels[i:min(i+batch_size,len(labels))]
    
                    batch_prompts = []
                    for label in batch_labels:
                        prompt = self.ice_template.generate_label_prompt_item(NA, ice, label, False)
                        batch_prompts.append(prompt)
    
    
                    batch_loss = self.cal_ce(batch_prompts, mask_length=mask_length)
    
                    loss_list.extend(batch_loss)
                probs = np.exp(-np.array(loss_list))
                normalized_probs = probs / probs.sum(0, keepdims=True)
                mdl_scores.append(entropy(normalized_probs, label_dim=0))
    ...
    def cal_ce(self, input_texts: List[str], mask_length=None):
        ...
        if self.metric_model is None:
            ...
            self.ce_tokenizer.pad_token = self.ce_tokenizer.eos_token
        ...
        self.hf_device_map = self.metric_model.hf_device_map
        self.device = next(iter(self.hf_device_map.values()))
        inputs = self.ce_tokenizer(input_texts, padding=True, return_tensors='pt', truncation=True)
        ...
        if mask_length is not None:
            if loss.shape[-1] - mask_length >= 0:
                ...
                mask = torch.zeros([loss.shape[0], loss.shape[-1]], dtype=torch.float)
                mask = mask.to(self.device)
